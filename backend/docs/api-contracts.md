...
- Auth / roles: сейчас не требуется
- Params: нет
- Query: нет
- Body DTO: нет
- Response DTO:
...
- Возможные ошибки:
- явные прикладные ошибки в коде не заданы
- `500 Internal Server Error`
- Пагинация / фильтрация / сортировка: не применимо
- Примечания:
- в коде есть TODO, нужен ли здесь `auth`
- контракт можно считать частично нестабильным
...
- `POST /api/auth/logout` имеет TODO про необходимость auth
